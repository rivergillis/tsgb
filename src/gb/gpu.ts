import { LOG, ERR, LOGV } from './helpers';
...
class Gpu {
  ...
  constructor(ctx: CanvasRenderingContext2D) {
    // Then init the framebuffer using the context
    this.frameBuffer = ctx.createImageData(160, 144);
  }
  ...
  reset = () => {
    ...
    for (let i = 0; i < this.frameBuffer.data.length; i += 4) {
      // Modify pixel data
      this.frameBuffer.data[i + 0] = 10; // R value
      this.frameBuffer.data[i + 1] = 50; // G value
      this.frameBuffer.data[i + 2] = 80; // B value
